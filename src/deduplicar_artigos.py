...
Antes de rodar:
    ...
    - Salve os arquivos na pasta dados/export_lens/
    - Nomeie como: string-01.csv, string-02.csv, ..., string-35.csv
    - Strings #26-#28: buscas-referência (QAmplifyNet / Hybrid QNN)
    - Strings #29-#35: rastreamento por área de aplicação ML em Supply Chain
"""
...
PASTA_CSV = os.path.join(PASTA_PROJETO, "data", "export_lens")
...
STRINGS_BUSCA = {
    ...
    # Causa raiz: strings amplas sem termo de supply chain (QML + Forecasting/Prediction/Time Series)
    ...
    # Rastreamento por área de aplicação ML em Supply Chain (#29-#35)
...
def extrair_numero_string(nome_arquivo):
    ...
    Exemplo: 'string-01.csv' -> 1, 'string-14.csv' -> 14
    Tambem aceita: '01.csv', 'busca-01.csv', etc.
    """
...
def carregar_csvs(pasta):
    ...
    # Strings removidas do pipeline (precisao < 2% para supply chain)
    STRINGS_EXCLUIDAS = {10, 11, 12, 13, 14}
    ...
    for arquivo in arquivos:
        ...
        # Pular strings removidas
        if numero in STRINGS_EXCLUIDAS:
            print(f"  {nome:30s} -> PULADO (string #{numero} removida)")
            continue
...
def deduplicar(df):
    ...
    # --- Etapa 5: Deduplicar por titulo normalizado (cross-DOI) ---
    # Artigos com DOIs diferentes mas mesmo titulo (ex: versoes preprint + journal)
    removidos_titulo_cross = 0
    if col_titulo and len(df_final) > 0:
        antes_cross = len(df_final)
        df_final["titulo_normalizado"] = (
            df_final[col_titulo]
            .fillna("")
            .str.lower()
            .str.strip()
        ...
        df_final = df_final.drop_duplicates(subset=["titulo_normalizado"], keep="first")
        df_final = df_final.drop(columns=["titulo_normalizado"])
        removidos_titulo_cross = antes_cross - len(df_final)
        if removidos_titulo_cross > 0:
            print(f"Deduplicacao por titulo (cross-DOI): {antes_cross} -> {len(df_final)} ({removidos_titulo_cross} duplicatas removidas)")
    ...
    stats = {
        ...
        "removidos_por_titulo_cross_doi": removidos_titulo_cross,
...
def imprimir_resultado_final(stats, resumo_strings):
    ...
    print(f"    - por titulo (cross-DOI):    {stats.get('removidos_por_titulo_cross_doi', 0):>6}")
...
def main():
    ...
    print(f"Proximo passo: abrir 'data/artigos_unicos.csv' para revisar os artigos.\n")
